...
---
background-image: url("capa_julho.png")
...
ipca_indice <- read.csv2("tabela1737.csv")

colnames(ipca_indice) <- c("date","Valor")

ipca_indice$date <- dmy(ipca_indice$date)
...
tabela_dolar <- data.frame(Período = c("Mês", "12 meses"),
                           Variação = c("-4,90%","-3,16%"))


tabela_comex_1 <- data.frame(Período = c("Mês", "12 meses"),
                           `Variação (exportação)` = c("+7,10%","+60,74%"),
                           `Variação (importação)` = c("+1,11%","+62,56%"))

tabela_comex_2 <- data.frame(Período = c("Mês", "12 meses"),
                           `Variação (exportação)` = c("-2,29%","+76,95%"),
                           `Variação (importação)` = c("+6,60%","+60,68%"))

tabela_comex_3 <- data.frame(Período = c("Mês", "12 meses"),
                           `Variação (exportação)` = c("+9,43%","+70,34%"),
                           `Variação (importação)` = c("+4,71%","+60,76%"))

tabela_comex_4 <- data.frame(Período = c("Mês", "12 meses"),
                           `Variação (exportação)` = c("+15,53%","+51,76%"),
                           `Variação (importação)` = c("-2,30%","+57,51%"))

tabela_comex_5 <- data.frame(Período = c("Mês", "12 meses"),
                           `Variação (exportação)` = c("-5,89%","+70,67%"),
                           `Variação (importação)` = c("+20,05%","+91,84%"))


tabela_aviacao <- data.frame(Período = c("Mês","12 meses"), 
                             `Variação (saídas)` = c("+18,34%","+268,61%"),
                             `Variação (chegadas)` = c("+30,06%","+245,31%"))
...
```{r echo=FALSE}
 ...
 ggplot()+
  ...
  scale_x_date(date_labels = "%b\n%Y", breaks = "12 month", expand = c(0.025,0.05)) +
...
ggplot(opcoes_dolar, aes(x= Date, y = valores, colour = meses)) +
  geom_line(size=1) +
  scale_x_date(date_labels = "%b\n%Y", breaks = "2 month") +
   scale_y_continuous(limits=c(4600, 6000),
                      breaks=seq(4600,6000, 300))+
...
ggplot(comex) + 
  ...
  scale_x_date(date_labels = "%b\n%Y", breaks = "1 months", labels = date_format("%b\n%y"), 
               limits = as.Date(c("2020-01-01","2021-06-10"))) +
  theme_minimal() +
  theme(legend.position = "bottom",
        axis.title.y = element_text(size=8),
        plot.title = element_text(size=12, hjust = 0.5, face="bold"),
...
exp_prod_graph <- ggplot(exportacao_produtos_2021, aes(x = total_bilhoes,  y = reorder(produto, +total_bilhoes))) + 
  ...
  scale_x_continuous(labels = scales::dollar_format(prefix = "US$",big.mark = ".", decimal.mark = ","),
                     ...
                     limits = c(0,10000)) + 
  labs(title = "5 produtos mais exportados no Brasil\n em Junho de 2021 (em milhões de dólares)", y = "", x="US$ (milhões)",
...
knitr::kable(head(tabela_expor_comex),
             ...
             align = "c",
             col.names = c("Produto",
                           "Valor exportado em Junho/21 (em milhões US$)",
                           "Variação (12 meses)",
                           "Variação mensal"),
             caption = "Referência: Junho/2021")
...
knitr::kable(head(tabela_impor_comex),
             ...
             align = "c",
             col.names = c("Produto",
                           "Valor importado em Junho/21 (em milhões US$)",
                           "Variação (12 meses)",
                           "Variação mensal"),
             caption = "Referência: Junho/2021")
```
